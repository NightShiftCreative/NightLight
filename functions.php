...
function ns_core_admin_scripts() {
    if (is_admin()) {
        ...
        wp_enqueue_style('ns-core-theme-options',  get_template_directory_uri() . '/admin/theme-options/theme-options.css', array(), '3.0', 'all');
    }
}
...
function ns_core_load_stylesheets() {
	if (!is_admin()) {
		...
		// enqueue styles
		wp_enqueue_style('bootstrap',  get_template_directory_uri() . '/css/bootstrap-grid.min.css', array(), '3.0', 'all');
...
if(class_exists('NS_Basics_Admin')) { 
